...
try {
	...
	for(let i = 0; i < mainVideos.length; i++, currentDirectoryNumber++) {
		...
		for(let j = 0; j < VARIATION_COUNT; j++) {
			...
			let cmd = `start ffmpeg -i ${mainVideoPath} -i ${musicPath} -f lavfi -i "color=${selectedColor.hex}:s=1080x1920" -c:a aac -c:v libx264 -filter_complex "[0:v][2:v]blend=shortest=1:all_mode=overlay:all_opacity=0.1[v]; [v]subtitles=${subtitlesFilePath.replaceAll(/\\/g, '\\\\\\\\').replaceAll(/:/g, '\\\\:')}:fontsdir=${fontFilesDirectoryPath.replaceAll(/\\/g, '\\\\\\\\').replaceAll(/:/g, '\\\\:')}:force_style='Alignment=2,MarginV=70,Fontname=${fontName},PrimaryColour=${selectedColor.ass}'[vout]; [0:a][1:a]amix=inputs=2:duration=shortest[aout];" -map "[vout]" -map "[aout]" ${join(currentOutputDirectoryPath, filename)}`
